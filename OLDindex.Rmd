...
The following function will install and load the required packages for this project. If you prefer to load them manually from your own library, the packages used are: `tidyverse`, `patchwork`, `ggplot2`, `bslib`, `kableExtra`, and `dplyr`.
...
First, load the raw data. The dataset is located in the `/data` folder of the `McQueen_Final_Project` repository. Be sure to replace `'data/wishes_pro_data_current.csv'` with the appropriate path to your CSV file.
...
The *WISHeS* dataset is large, comprising over 237 variables for 220 participants. For this visualization project, I focused specifically on participants' age and sex to simplify the analysis and highlight patterns and trends in stroke incidence related to these factors. By creating a reduced dataset, I was able to streamline the information, making it more manageable and aligned with the project's objectives. The full details of the data cleaning and preprocessing steps are outlined below.
...
Stroke incidences peak for males in the 55-59 age group before declining, whereas females reach their highest incidence in the 65-69 age group, followed by a gradual decrease with age. These findings highlight the need to prioritize stroke prevention efforts for males in their 50s and early 60s and for females in their late 60s and beyond. Overall, the final visualization supports the relationship between age, biological sex, and stroke incidence, aligning with existing research and providing key insights for public health strategies and interventions.
