---
title: "**McQueen Final PSY6422 Project**"
date: "2024-11-28"
output:
  html_document:
    theme: sandstone
...
Stroke is a leading cause of death and disability worldwide, with outcomes that vary significantly across sex and age groups. Research has highlighted that these differences stem from a complex interplay of biological, hormonal, and behavioral factors (Appelros et al., 2009; Bushnell et al., 2014). Men tend to experience strokes at younger ages with higher incidence rates, whereas women, who typically experience strokes later in life, often face worse long-term recovery outcomes (Bushnell et al., 2018). This pattern is further influenced by the higher prevalence of strokes among older individuals, especially women, due to their longer life expectancy (Gargano & Reeves, 2007).

The **Women’s Imaging of Stroke Hemodynamics Study** (WISHeS) is a prospective dataset comprising 220 participants, designed to examine how sex differences in cerebrovascular and hemodynamic factors influence stroke outcomes. By analyzing this dataset, researchers can gain valuable insights into the role of sex-specific biological markers and their impact on stroke recovery. For this project, I aim to explore the incidence of stroke across various age groups by gender, using the *WISHeS* dataset to determine whether the observed trends align with those documented in the literature. This analysis will contribute to a deeper understanding of the relationship between sex, age, and stroke outcomes.
...
The McQueen_Final_Project repository is organized into key sections to help you navigate its contents. The **`/codebook`** folder provides detailed documentation on the dataset, including variable descriptions and structure, offering essential context for the analysis. The **`/data`** folder contains the raw datasets used in this project, forming the basis of all analyses. The **`/figures`** folder showcases visualizations and plots created during the analysis, highlighting the project's key findings and insights. Lastly, the **`/scripts`** folder includes all the code used for data processing, analysis, and visualization. Together, these sections guide you through the project workflow, from raw data to final outputs.

# **Dataset**

## Dataset Origins

The raw data for this visualization project comes from the Lone Star Stroke Research Consortium as part of the **Women’s Imaging of Stroke Hemodynamics Study** (WISHeS), led by Principal Investigator Dr. Adrienne N. Dula at The University of Texas at Austin. Permission to use this dataset for this project was kindly granted by Dr. Dula. The *WISHeS* dataset focuses on exploring sex differences in cerebrovascular and hemodynamic factors to understand their role in stroke outcomes. The dataset includes a comprehensive range of clinical information—such as demographics, medical history, and behavioral risk factors—alongside detailed imaging data, including vascular territories, Circle of Willis variations, and ischemic volumes.

## Potential Limitations of Dataset

The dataset from the *WISHeS* offers a strong foundation for analyzing cerebrovascular and hemodynamic factors, but it has certain limitations. Most notably, its focus on a specific study population may restrict the generalizability of findings to broader groups, particularly those with diverse geographic, ethnic, or socioeconomic backgrounds. These considerations are important when interpreting the project's results.
...
library(bslib)      # Provides tools for themeing Shiny applications
...
The WISHeS dataset is large, comprising over 237 variables for 220 participants. For this visualization project, I focused specifically on participants' age and gender to simplify the analysis and highlight patterns and trends in stroke incidence related to these factors. By creating a reduced dataset, I was able to streamline the information, making it more manageable and aligned with the project's objectives. The full details of the data cleaning and preprocessing steps are outlined below.
...
## Descriptiion of Cleaned Data

The reduced WISHeS dataset focuses on three key variables: **patient_id**, **jc_gender**, and **gs_age**. The **patient_id** variable uniquely identifies each participant, ensuring the data remains organized and distinct for analysis. The **jc_gender** variable records participants' gender, allowing for comparisons of stroke incidence between males and females. Lastly, the **gs_age** variable represents participants' ages, enabling an exploration of age-related patterns in stroke incidence. These variables form the core of the analysis, providing a streamlined and targeted approach to understanding the relationship between age, gender, and stroke incidence.

## Creating Age Groups

This project aims to analyze stroke incidence among males and females across different age groups. Creating defined age groups allows for a more structured and meaningful comparison, making it easier to identify trends and patterns that might not be evident when analyzing individual ages. To establish these groups, I first determined the minimum and maximum participant ages in the dataset, ensuring the age ranges accurately represent the full spectrum of participants. This method ensures a comprehensive and systematic analysis of stroke incidence across age and gender.
...
Based on the dataset's minimum and maximum ages, I organized participants into age groups using 5-year intervals to ensure consistent and meaningful comparisons. This was accomplished by adding an **`age_group`** variable to the dataset, which assigns each participant to a specific age range based on their recorded age. The age groups span from "0-19" to "100+" and are clearly labeled to represent each interval accurately. This structured approach provides a solid foundation for analyzing trends across different age categories.
...
For the initial visualization, I used a ggplot bar graph to compare stroke incidence between males and females across different age groups. The bar graph was designed with side-by-side bars for each gender within each age group, allowing a direct comparison. While the graph provided a basic overview of the data, it had several limitations that made it less effective for highlighting key insights.
...
ggplot(reduced_Wishes_data, aes(x = age_group, fill = factor(jc_gender))) + 
  ...
  labs(
    ...
    fill = "Gender"      # Label the legend as "Gender"
  ) +
...
### Description of the Second Visualization
...
While this visualization enhanced the clarity of gender-based differences, I identified an area for improvement: the placement of age groups along the y-axis. Positioning the age groups in the center of the pyramid, rather than on the sides of the y-axis, could make the comparison more intuitive and visually cohesive. This adjustment would further emphasize the alignment between the two genders and create a cleaner, more accessible design for interpreting trends in stroke incidence across age groups.
...
ggplot(age_gender_count, aes(x = count, y = age_group, fill = factor(jc_gender))) +
  ...
  labs(
    ...
    fill = "Gender"                 # Label the legend as "Gender" for clarity.
  ) +
...
data <- data %>%       # Use the pipe operator (%>%) to apply multiple transformations in sequence   
  mutate(              # Create or modify columns in the dataset
    age_group = cut(   # Categorize the 'gs_age' variable into age groups
      ...
      right = FALSE    # Use 'right = FALSE' to exclude the upper limit of each interval
    ),
    gender = recode(jc_gender, `1` = "Male", `2` = "Female")   # Create a new variable 'gender'. Map '1' to "Male" and '2' to "Female"
  ) %>%
  drop_na(age_group, gender)   # Remove rows with missing values in the 'age_group' or 'gender' columns
```
...
combined_plot <- plot_male + plot_labels + plot_female +   # Combine 'plot_male', 'plot_labels', and 'plot_female' side-by-side
  ...
  plot_annotation(
    ...
    caption = "Data Source: Women’s Imaging of Stroke Hemodynamics Study",              # Add a caption identifying the data source
    theme = theme(
...
The final visualization compares the number of stroke incidences between males and females across various age groups, highlighting notable trends and disparities. Stroke incidences generally increase with age for both genders, reflecting the higher risk of strokes in older populations. Among younger age groups (below 55), males tend to experience more strokes than females, although the overall numbers in these groups remain relatively low. In older age groups, the pattern reverses, with females experiencing more strokes than males. This trend persists into the oldest ages (85 and above), likely driven by the longer life expectancy of females.

For males, stroke incidences peak in the 55-59 age group before declining, whereas females reach their highest incidence in the 65-69 age group, followed by a more gradual decrease with age. The pronounced peak in females during this period potentially highlight a critical window for targeted interventions. These findings suggest prioritizing stroke prevention efforts for males in their 50s and early 60s and for females in their late 60s and beyond. Overall, the visualization emphasizes the impact of age and gender on stroke patterns, offering essential insights for public health planning and intervention strategies.

It is important to acknowledge the limitations of this data when interpreting the findings. The visualization is based on data from the *Women’s Imaging of Stroke Hemodynamics Study*, which includes only 220 patients—a relatively small sample size that may limit the generalizability of the results to larger or more diverse populations. Additionally, while the analysis focuses on age and gender as key factors influencing stroke incidences, it does not account for other critical variables that can significantly impact stroke risk. Factors such as lifestyle, diet, genetic predisposition, socioeconomic status, access to healthcare, and preexisting health conditions (e.g., hypertension, diabetes) were not considered in this visualization. These unmeasured variables could introduce bias or confounding effects, underscoring the need for caution when drawing broad conclusions or applying these insights to different populations. Further research with larger, more representative samples and a more comprehensive analysis of contributing factors is needed to fully understand the dynamics of stroke risk.

# **Future Studies**

Future studies analyzing the *WISHeS* dataset could explore additional visualizations to uncover further insights. For instance, a scatter plot could be designed to explore the relationship between stroke severity and age, using NIHSS scores as the measure of stroke severity. By adding color coding or shape differentiation for **jc_gender**, researchers could identify potential differences in stroke severity trends between males and females. These analyses would facilitate a more nuanced understanding of how age and gender interact with stroke severity, offering valuable insights for future research and clinical interventions.
