#---------------------------------
# Final Script for PSY6422 Project
#---------------------------------
...
ggplot(reduced_Wishes_data, aes(x = age_group, fill = factor(jc_gender))) +
  ...
  scale_fill_manual(values = c("#1C3A04", "#7eb200")) +  # Assign custom colors to 'male' and 'female'
...
summary_data <- data %>%
  group_by(age_group, sex) %>%  # Group data by age group and biological sex
  summarise(count = n(), .groups = 'drop') %>%  # Count the number of rows in each group, dropping extra grouping
  pivot_wider(names_from = sex, values_from = count, values_fill = 0) %>%   # Reshape data to have separate columns for Male and Female, populate the reshaped table with counts, fill missing values with 0  
...
combined_plot <- plot_male + plot_labels + plot_female +
  ...
  plot_annotation(
    ...
    caption = "Data Source: Women’s Imaging of Stroke Hemodynamics Study (WISHeS)",  # Add a caption for the reference 
    theme = theme(
